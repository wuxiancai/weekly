...
- Web 页面 `STRATEGY_DEFAULTS.BTCUSDT['4h']` 已更新为当前默认：
  - `EMA8 / MA35`
  - `ADX >= 14`
  - 多头 RSI `50-80`，空头 RSI `0-100`
  - 止损 `0.8 ATR`
...
- 当前默认回测结果，`BTCUSDT 4h`，实际数据 `2019-09-08` 至 `2026-06-29`：
  ...
  - 胜率 `24.11%`
  - 盈利因子 `1.289`
- 当前判断：这不是计算口径 bug，但策略形态偏激进，交易次数过多、胜率偏低，收益依赖复利和少数大盈利交易，不宜只看总收益率。
- 本轮找到的更平衡候选：
  - `EMA8 / MA35`
  - `ADX >= 25`
  - 多头 RSI `50-80`，空头 RSI `0-100`
...
- 平衡候选回测结果：
  ...
  - 胜率 `25.22%`
  - 盈利因子 `1.502`
- 结论：如果后续要把 4h 默认改得更健康，优先考虑这组平衡候选；它收益略高于当前默认，同时交易次数接近减半、盈利因子明显更高、权益曲线回撤更低。
