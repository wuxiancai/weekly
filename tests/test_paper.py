...
class PaperTradingTests(unittest.TestCase):
    def test_paper_defaults_use_shared_1000_usdt_account_and_all_strategy_intervals(self) -> None:
        ...
        self.assertEqual(params_by_key[("BTCUSDT", "15m")].ema_period, 34)
        self.assertEqual(params_by_key[("BTCUSDT", "15m")].ma_period, 89)
        self.assertEqual(params_by_key[("BTCUSDT", "15m")].adx_min, 45)
        self.assertEqual(params_by_key[("BTCUSDT", "15m")].take_atr, 10.0)
        self.assertEqual(params_by_key[("BTCUSDT", "15m")].take_atr_step, 2.0)
        self.assertEqual(params_by_key[("BTCUSDT", "15m")].take_atr_max, 24.0)
        self.assertEqual(params_by_key[("BTCUSDT", "15m")].volume_mult, 1.75)
        ...
        self.assertEqual(params_by_key[("ETHUSDT", "15m")].long_rsi_min, 45)
        self.assertEqual(params_by_key[("ETHUSDT", "15m")].take_atr, 10.0)
        self.assertEqual(params_by_key[("ETHUSDT", "15m")].take_atr_step, 2.0)
        self.assertEqual(params_by_key[("ETHUSDT", "15m")].take_atr_max, 24.0)
        self.assertEqual(params_by_key[("ETHUSDT", "15m")].trend_ma_gap_min, 0.003)
    ...
    def test_paper_page_shows_intervals_amounts_and_formatted_log_times(self) -> None:
        ...
        self.assertIn("<th>止损</th><th>保护/止盈</th><th>预计盈利</th><th>预计收益率</th><th>最新止盈</th>", PAPER_HTML)
        ...
        self.assertIn('<td class="pos">${formatPrice(p.initial_take_price)}</td>${projectedTakeProfitCell(p)}${projectedTakeProfitRateCell(p)}<td class="pos">${formatPrice(p.latest_take_price)}</td>', PAPER_HTML)
        ...
        self.assertIn("function projectedTakeProfitRate(p)", PAPER_HTML)
        self.assertIn("return pnl / margin * 100;", PAPER_HTML)
        self.assertIn("function projectedTakeProfitRateCell(p)", PAPER_HTML)
    ...
    def test_paper_summary_grid_uses_adaptive_widths_for_capital_allocation(self) -> None:
        ...
        self.assertIn("grid-template-columns:minmax(150px,max-content) minmax(140px,max-content) minmax(92px,max-content) minmax(560px,1fr) max-content max-content", PAPER_HTML)
        self.assertIn(".summary-compact { padding-left:14px; padding-right:14px; }", PAPER_HTML)
    ...
    def test_paper_summary_shows_runtime_duration_after_strategy_intervals(self) -> None:
        ...
        self.assertIn(".strategy-intervals, .runtime-duration { display:grid; gap:2px; font-size:18px;", PAPER_HTML)
        self.assertIn("document.getElementById('runtimeDuration').innerHTML = formatRuntimeDuration(account.started_at);", PAPER_HTML)
