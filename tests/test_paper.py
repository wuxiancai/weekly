...
class PaperTradingTests(unittest.TestCase):
    def test_paper_defaults_use_shared_1000_usdt_account_and_4h_strategies(self) -> None:
        strategies = paper_strategy_defaults()

        self.assertEqual(PAPER_DEFAULT_INITIAL_EQUITY, 1000.0)
        self.assertEqual([(s.symbol, s.interval) for s in strategies], [("BTCUSDT", "4h"), ("ETHUSDT", "4h")])
        self.assertTrue(all(s.params.regime_switch for s in strategies))
    ...
    def test_paper_engine_initializes_account_and_strategies_once(self) -> None:
        ...
        strategies = conn.execute("SELECT * FROM paper_strategies ORDER BY symbol").fetchall()
